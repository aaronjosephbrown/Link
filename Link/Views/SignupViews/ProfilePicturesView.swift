...
struct ProfilePicturesView: View {
    ...
    private func uploadPhotos() {
        ...
        group.notify(queue: .main) {
            if uploadedUrls.count == selectedImages.count {
                ...
                db.collection("users").document(userId).updateData([
                    ...
                    appViewModel.updateProgress(.photosComplete)
                    currentStep += 1
                }
            }
        }
...
#Preview {
    ProfilePicturesView(isAuthenticated: .constant(false), currentStep: .constant(1))
